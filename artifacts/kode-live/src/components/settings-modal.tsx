...
import {
  ...
  BellIcon, CheckIcon, ChevronRightIcon, GithubIcon, ExternalLinkIcon,
  EyeIcon, EyeOffIcon, Loader2Icon, SaveIcon,
} from "lucide-react";
...
// ── Profile Tab ───────────────────────────────────────────────────────────────
function ProfileTab() {
  const [name, setName] = useState("Хөгжүүлэгч");
  const [email, setEmail] = useState("dev@kodu.live");
  const [saved, setSaved] = useState(false);

  const save = () => {
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
    toast({ title: "Профайл хадгалагдлаа" });
  };
    ...
    <div className="flex flex-col gap-6">
      <Section title="Хэрэглэгчийн мэдээлэл">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <Button variant="outline" size="sm" className="font-mono text-xs h-7">Зураг солих</Button>
            <p className="text-[10px] font-mono text-muted-foreground mt-1">PNG, JPG — 2MB хүртэл</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} className="font-mono text-sm h-8 bg-card/50" />
          </div>
          <div>
            <label className="text-[10px] font-mono text-muted-foreground uppercase tracking-wide mb-1.5 block">Email</label>
            <Input value={email} onChange={(e) => setEmail(e.target.value)} className="font-mono text-sm h-8 bg-card/50" />
          </div>
        </div>
      </Section>
    ...
    <div className="flex flex-col gap-6">
      ...
      <Section title="Зан байдал — Behavior">
        ...
        <Row label="Streaming хариулт" sub="Бичиж байгаа мэт шуурхай харуулна">
          <Toggle on={streaming} onChange={setStreaming} />
        </Row>
      </Section>
      ...
      <div className="flex items-center gap-2 bg-primary/5 border border-primary/20 rounded-lg px-3 py-2.5">
        ...
        <p className="text-[11px] font-mono text-muted-foreground">
          Одоогийн тохиргоо: <span className="text-foreground">{model}</span> · {tokens} tokens · {lang === "mn" ? "Монгол" : lang === "en" ? "English" : "Хоёулаа"}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <Section title="Харагдах байдал — Appearance">
        ...
        <Row label="Загвар" sub="Dark / Light / System">
          <Select
            value={theme}
            onChange={setTheme}
            options={[
      ...
      <Section title="Засах тохиргоо — Editing">
        ...
        <Row label="Minimap" sub="Кодын тойм харуулах">
          <Toggle on={minimap} onChange={setMinimap} />
        </Row>
      </Section>
    </div>
  );
...
function ApiKeysTab() {
  ...
  const connectGh = () => {
    setGhLoading(true);
    setTimeout(() => { setGhLoading(false); setGhConnected(true); toast({ title: "GitHub холбогдлоо ✓" }); }, 1500);
  };
  const connectVercel = () => {
    setVercelLoading(true);
    setTimeout(() => { setVercelLoading(false); setVercelConnected(true); toast({ title: "Vercel холбогдлоо ✓" }); }, 1500);
  };
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] font-mono text-muted-foreground mt-0.5">Өөрийн Claude API key ашиглах</p>
            </div>
            <Badge variant="outline" className="text-[9px] font-mono">Заавал биш</Badge>
          </div>
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              >
                {showAnthropic ? <EyeOffIcon className="w-3.5 h-3.5" /> : <EyeIcon className="w-3.5 h-3.5" />}
              </button>
            </div>
            <Button size="sm" variant="outline" className="font-mono text-xs h-8 shrink-0">
              Хадгалах
            </Button>
          </div>
          {!anthropicKey && (
            <p className="text-[10px] font-mono text-muted-foreground/60">
              Key оруулаагүй бол kodu.live-ийн key ашиглана (rate limited)
            </p>
          )}
        </div>
      </Section>
        ...
        <div className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-card/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-mono text-muted-foreground">Repository sync · Commit</p>
            </div>
          </div>
          {ghConnected ? (
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1 text-[10px] font-mono text-emerald-400">
                <CheckIcon className="w-3 h-3" /> Холбогдсон
              </span>
              <Button size="sm" variant="outline" onClick={() => setGhConnected(false)} className="font-mono text-[10px] h-6 px-2 text-muted-foreground">
                Салгах
              </Button>
            </div>
          ) : (
            <Button size="sm" onClick={connectGh} disabled={ghLoading} className="font-mono text-xs h-7 gap-1.5">
              {ghLoading ? <Loader2Icon className="w-3 h-3 animate-spin" /> : <ExternalLinkIcon className="w-3 h-3" />}
              Холбох
            </Button>
          )}
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-card/20">
          ...
          {vercelConnected ? (
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => setVercelConnected(false)} className="font-mono text-[10px] h-6 px-2 text-muted-foreground">
                Салгах
              </Button>
            </div>
          ) : (
            <Button size="sm" onClick={connectVercel} disabled={vercelLoading} className="font-mono text-xs h-7 gap-1.5">
              {vercelLoading ? <Loader2Icon className="w-3 h-3 animate-spin" /> : <ExternalLinkIcon className="w-3 h-3" />}
              Холбох
            </Button>
          )}
        </div>
      ...
      <Section title="Аюулын бүс">
        <div className="flex flex-col gap-3 p-4 rounded-lg border border-destructive/30 bg-destructive/5">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-mono font-semibold text-destructive">Акаунт устгах</p>
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button
              ...
              onClick={() => toast({ title: "Акаунт устгагдлаа", variant: "destructive" })}
            >
              Бүрмөсөн устгах
            </Button>
          </div>
